...
| `scanner.py` | Fetches markets, detects pairs, validates orderbook depth | `CandidatePair`, `ApiMarket`, `normalize_title()`, `pair_key()`, `display_title()`, `fetch_open_events_with_markets()`, `fetch_shard_statuses()`, `check_shard_coverage()`, `find_time_series_pairs()`, `find_same_title_pairs()`, `enrich_with_orderbook_prices()`, `filter_markets_within_horizon()`, `get_held_tickers()` |
| `strategy.py` | Kelly sizing, portfolio selection | `TradeSpec`, `compute_trade()`, `select_portfolio()` |
| `trader.py` | Order submission with atomic rollback, plus per-shard collateral funding | `execute_trades()`, `pre_execution_check()`, `ensure_shard_collateral()` |
...
**Per-run shard coverage check.** `scanner.check_shard_coverage(advertised, market_shards, balance_shards)` is a pure function (no I/O, no logging) that compares the shards `/exchange/status` advertises against what a run actually observed, and returns `(critical, warnings)` problem-string lists. `main._log_shard_coverage` (called from both `_run_dev` and `_run_prod`, right after the market fetch) logs each `critical` entry via `logging.critical("SHARD COVERAGE FAILURE: %s", ...)` — the same severity channel as orphaned positions, so it's never missable — and each `warning` via `logging.warning`; with neither, and the advertised breakdown present, it logs `"Full shard coverage: shards [...] scanned"`. A run must only ever CLAIM full coverage when every shard the exchange advertises was actually scanned. The severity split is deliberately empty-vs-funded: an advertised `trading_active=True` shard with zero ingested markets is CRITICAL only when it also holds a nonzero balance (a real blind spot — money on a shard whose book we never scanned could hide an undetected pair); with no funds there it's a WARNING, since shard 1 is near-empty today and shards 2/3 are advertised-but-empty until the 2026-08-24 migrations — an empty advertised shard must not cry wolf at critical severity every week. Markets ingested from a shard `/exchange/status` doesn't advertise at all are always CRITICAL (the two data sources disagree, independent of money); a balance on an unadvertised shard is a WARNING (funds with no market activity to miss aren't a missed arbitrage opportunity). A `trading_active=False` advertised shard is never flagged — its markets are dropped deliberately at ingest, with their own warning. `advertised is None` (status breakdown unavailable) makes coverage unknowable and returns `([], [])` unconditionally; the caller logs at INFO that coverage isn't assessable rather than claiming either full or broken coverage. `balance_shards` must be pre-filtered to nonzero balances by the caller (`{s for s, c in shard_balances.items() if c > 0}`) — the function only checks set membership, so a zero-cent breakdown entry must not read as "funds present." Dev mode passes `balance_shards=set()` (no real per-shard breakdown, one virtual balance) so only the market-coverage half of the check is meaningful there. The check never aborts the run — a missing shard is reported loudly but trading continues on whatever was covered.
...
**Collateral is per-shard; the bot auto-transfers before execution (2026-08-15).** Sizing is portfolio-wide (Kelly on the summed cross-shard balance) but an order settles against ITS OWN market's shard, so a selected trade can need cash on a shard that doesn't hold enough. `trader.ensure_shard_collateral(client, portfolio, shard_balances, shard_statuses, dry_run)` runs in `main._run_prod()` between `pre_execution_check()` and `execute_trades()` (prod only — dev has no real shards or balance breakdown). It totals each shard's requirement from the legs' `cost_with_fees_a`/`cost_with_fees_b` against `market_a`/`market_b`'s `exchange_index` (`math.ceil` to cents, `round(...,6)` first like `fee_leg_exact` — **never floor**, an under-funded shard means an insufficient-collateral rejection), plans greedy transfers with the pure `_plan_transfers()` (deficits in shard order, each filled from the largest *remaining* surplus first so the plan is deterministic and uses the fewest POSTs), and POSTs each to `config.TRANSFER_PATH` via `_http.signed_raw_request` + `fetch_json_page`. Load-bearing details, all of them: **(1) CENTICENTS.** The endpoint's `amount` is int64 centicents (1/100 of a cent = cents × 100) — the codebase's THIRD money unit after integer cents and fixed-point dollar strings, and it lives behind `trader._cents_to_centicents()` alone. Never inline the factor; getting it wrong moves 1% or 100× of the intended collateral. **(2) Asynchronous settlement.** A 2xx means *accepted*, not settled — no order may rely on the funds until a fresh balance read shows them. After the POSTs, `_await_transfer_settlement()` re-polls `auth.verify_auth()` every `TRANSFER_POLL_INTERVAL_SECONDS` (2s) until every deficit shard is covered or `TRANSFER_SETTLE_TIMEOUT_SECONDS` (30s) elapses; on timeout it logs `logging.critical` naming the in-flight `transfer_id`s ("money is in flight — check the account") and drops ONLY the specs whose shards stayed unfunded. **(3) No retry, ever.** The transfer endpoint is NOT idempotent, so a retry after an ambiguous failure moves the money twice — a raised POST is logged at ERROR, its shard stays unfunded, and the next run re-plans from real balances. `trader.py` deliberately does not even import `api_call_with_retry` (a test asserts that). **(4) Transfers-active gate.** With a `shard_statuses` breakdown present, a planned transfer whose source *or* destination has `intra_exchange_transfers_active=false` is never attempted; affected specs are dropped with a "move funds manually in the Kalshi UI" warning. A shard absent from a present breakdown is treated as inactive (refusing costs a trade; guessing risks the money). `shard_statuses=None` (sandbox/pre-sharding) means attempt anyway and let the POST fail loudly. **(5) Dry run plans and logs but never POSTs**, and returns the portfolio unchanged. **(6) A spec is dropped iff ANY leg's shard is unfunded** — a payable leg A with an unpayable leg B is exactly the half-fill the rollback machinery exists to avoid. **(7) Zero-deficit fast path** returns the portfolio untouched with no API calls at all, and it keys off the deficit set, NOT off an empty plan — an empty plan is ambiguous ("nothing needed" vs. "no surplus to move"), and reading it as success would let unfunded trades through. Today the whole thing is a pure no-op: all funds and all tradeable markets are on shard 0, and `_legacy_routable` still refuses off-shard pairs anyway. It lands ahead of the V2 order flip so the funding path is already in place (and exercised via the fast path) when trades can actually route off shard 0.
