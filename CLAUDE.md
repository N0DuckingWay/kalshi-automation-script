...
- Tests live in `tests/` (`test_auth.py`, `test_backtester.py`, `test_config.py`, `test_dashboard.py`, `test_historical.py`, `test_http.py`, `test_main.py`, `test_reporter.py`, `test_scanner.py`, `test_scheduler.py`, `test_strategy.py`, `test_trader.py`) and run fully offline against `MagicMock` clients.
...
**Constants live in `config.py` exclusively.** Never hardcode `0.07` (fee rate), `0.20` (Kelly cap), `0.15`/`0.30` (tiered time-series price thresholds — always go through `min_price_diff_for_gap()`), `0.05` (same-title price threshold), `5000` (min balance cents), `1` (buy_max_cost slippage cents), `5` (max accepted per-contract rollback loss in cents — `ROLLBACK_MAX_LOSS_CENTS_PER_CONTRACT`), `8` (trader thread-pool size — `TRADER_MAX_WORKERS`, used by both `pre_execution_check` and `execute_trades`; also the value, not the same constant, of `SETTLED_FETCH_MAX_WORKERS` and `CANDLESTICK_FETCH_MAX_WORKERS`), `25` (scanner pagination progress cadence — `SCANNER_PROGRESS_LOG_EVERY_PAGES`), `50` (archive walk barren-page stop — `ARCHIVE_MAX_BARREN_PAGES`), `50_000` (day-slice worker flush size — `SETTLED_FETCH_CHUNK_RECORDS`), `0`/`10`/`20` (process exit codes — `EXIT_OK`/`EXIT_SKIPPED_LOW_BALANCE`/`EXIT_TRADES_NEED_ATTENTION`), or `3600` (scheduler job timeout) inline. Always import from `config.py`.
...
**`TradeResult.status` vocabulary:** `"executed"` (both legs filled, or leg B errored but the position delta was exactly `+spec.y`), `"simulated"` (dry run), `"failed"` (leg A confirmed unfilled — a clean FoK rejection, or an error with a delta of 0; nothing to unwind), `"rolled_back"` (leg B confirmed unfilled — FoK rejection, or an error with a delta of 0 — and leg A's unwind FoK filled), `"rollback_failed"` (unwind did not fill — orphaned position), `"manual_review"` (a leg errored and its outcome could NOT be attributed to this order: the position lookup failed after retries, or the position moved by an amount the order can't explain). `manual_review` submits no order at all — on leg B because unwinding could reverse a real fill, and on leg A because a `reduce_only` sell against a position this order may not own would liquidate an unrelated holding. Don't "simplify" `manual_review` into an automatic rollback in either leg.

**FoK price protection:** Both buy legs carry `buy_max_cost` = `count × (scanned price + BUY_MAX_COST_SLIPPAGE_CENTS)`, so an order fills at or below scanned-price-plus-slippage or not at all. Any new order-building code must set this cap. `trader._buy_max_cost_cents()` rounds the price product to 6 decimals *before* the ceiling (the `fee_leg_exact()` idiom) so binary float noise — `7 * 0.07 * 100 = 49.000000000000006` — can't ceil the cap a cent looser; this only ever tightens it. **The rollback carries the same kind of protection on the sell side.** `trader._rollback_leg_a()` unwinds leg A with a *floored* FoK **limit** sell (`type="limit"`, `no_price=_rollback_floor_cents(spec)`), not a market sell: `CreateOrderRequest` has no proceeds floor for market orders, so a book that collapsed between leg A's fill and the unwind would realize an unbounded loss. The floor is leg A's scanned entry (`spec.pair.nA`) less `ROLLBACK_MAX_LOSS_CENTS_PER_CONTRACT`, clamped into the API's valid `1..99` cent range, with `round()` applied *before* `int()` because `nA` is a cent-quantized book price carried as a float (`0.57 → 56.99999999999999`, which `int()` would truncate to 56). A book below the floor kills the unwind, leaving the leg-A position open — reported as `status="rollback_failed"` for manual review, exactly the path an unfilled market unwind already took, so the caller's contract is unchanged and only the loss is now bounded. Don't "simplify" the unwind back to `type="market"`.
