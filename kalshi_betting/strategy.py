...
def _kelly_p(pair: CandidatePair) -> float:
    ...
    scanner.find_time_series_pairs now SCREENS both legs' wording for
    (scanner.deadline_phrasing) rather than assuming, so a snapshot family
    ("price ON <date>", whose probabilities do not nest) never reaches this
    model. The YES-ask gap pB - pA is the market-implied probability of the
    loss cell; the model
