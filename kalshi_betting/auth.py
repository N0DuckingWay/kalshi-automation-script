...
def _balance_cents_from_payload(data: dict) -> int:
    ...
    preference order is:
      ...
      1. The balance_breakdown entry for DEFAULT_EXCHANGE_INDEX — the only
         shard the legacy order endpoint can reach until the V2 migration
         lands, so funds parked on any other shard must not inflate Kelly
         sizing. Entries carry the dollar string under the key "balance"
         (NOT "balance_dollars").
