...
backtest.py (CLI)
  ...
  │         │                              run's own per-trade size cap (config.BUDGET_FRACTION)
  │         └─ CapSweep (default; not with --no-cap-sweep) — every other per-trade size cap of
  │                                        SIZE_CAP_SWEEP, none of them simulated here: each
  │                                        (band, k) cell is simulated through
  ...
  └─ dashboard.generate_dashboard()          — write HTML report: a k section and two k̂ cards
                                                ...
                                                per-population KPIs), and a sticky filter bar whose
                                                every band x k x size cap x Kalshi category x tag view
                                                is computed here: one walk over the scenario grid reads
                                                each CapSweep cell (simulating it), packs one chunk per
                                                distinct trade list, keeps the k section's rows and
                                                curves at every k and cap and the explorer's figures
                                                for every band x k x cap (a block per cap), streamed
                                                into the page beside base blocks and unpacked by small
                                                scripts only when chosen
```
...
Explorer" section has no scenarios to show. The heatmap's metric menu
always offers nine metrics, among them the Sharpe ratio and `k̂ − k` (each
band's pooled empirical `k̂` minus the column's `k`; it and the `k̂` view
are both red where `k̂` exceeds `k`, as on the cards). With the size-cap
sweep on (the default, `--no-cap-sweep` the opt-out) the explorer also
offers every per-trade size cap, each cap's figures simulated as the
dashboard is built and shipped in a block of its own. The explorer's
...
sizing keeps reading `config.BUDGET_FRACTION`).** Every simulation above
sizes a trade at most at the run's own per-trade Kelly cap,
`config.BUDGET_FRACTION` (20% of the checkpoint's opening balance). The cap
sweep is **on by default** and offers every other cap of
...
largest uncapped Kelly fraction sizes its trades identically, so those caps
share one simulation — the run's own result itself when its 20% cap is at or
above that fraction, or else the first larger cap's fresh simulation (a
...
adds to the dashboard phase's time and page size. `--no-cap-sweep` returns no
