...
main.py
  ...
  │                                         tiered on their stated calendar deadlines)
...
Fetches the live account balance, scans real markets, submits fill-or-kill orders leg-by-leg, and appends results to `trade_log.xlsx`. By the 2026-09-26 decision this also pairs and sizes same-event deadline ladders (`config.TIME_SERIES_SAME_EVENT_LADDERS`); read that constant's comment and run the dry run below first.
...
Runs from 2024-01-01 with a $10,000 simulated balance, pairing same-event ladders at the configured switch (on by the 2026-09-26 decision). Results are cached in `backtest_cache/`.
