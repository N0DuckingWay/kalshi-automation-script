...
python3 -m kalshi_betting.backtest --no-cap-sweep            # skip the per-trade size-cap sweep (config.BUDGET_FRACTION only)
```
...
do). `--no-cap-sweep` returns no cap sweep at all. The grid
lives in `backtester.py` rather than `config.py` by the operator's
