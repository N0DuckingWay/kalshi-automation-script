...
config.py (constants), _http.py (retry + raw-response fetch)
  — both leaves: neither imports another project module —
    ...
    (historical.py also imports auth.py's build_client for its own client
     ...
     live scanner — plus deadline_profile/cumulative_deadline_pair, the one
     definition of what makes a pair a cumulative-deadline pair, and leg_sides
     so settlement pays by the side each leg bought)
...
| `backtester.py` | Replays the strategy on settled markets: groups them into candidate pairs, scans weekly Monday snapshots for the first tradeable entry, applies Kelly sizing, records actual P&L from settlement outcomes, and builds a daily equity curve that opens one day before the start date at the untouched initial balance, so a trade entering on the first day of the window shows its day-0 charges as a real daily return and a real drawdown. The curve is a portfolio value, not a cash balance: an open position is carried at its cost basis for its whole holding period, so committing capital does not move the curve and the drawdown/Sharpe/Sortino figures derived from it measure realized loss rather than peak deployment. |
