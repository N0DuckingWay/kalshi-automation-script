...
backtest.py (CLI)
  ...
  └─ dashboard.generate_dashboard()          — write HTML report: k-selector section, a k̂ breakdown by
                                                ...
                                                (band x k heatmap, per-population KPIs, and its
                                                tier-floors-off grid, shown when the bar's Tier floors
                                                choice is off), and a sticky
                                                filter bar whose every band x tier floors (on, or the
                                                tier-floors-off run) x Kalshi category x tag view is
                                                computed here, packed into the page and swapped in by
                                                a small script
```
...
the tier floors below; every population is labelled on the page. `--no-band-sweep` skips that
grid, and the tier-floors-off runs below with it: the primary scenario still
runs, but the dashboard's "Scenario Explorer" section has no scenarios to
show, and its filter bar offers the primary band only, with the Tier floors
select disabled. The explorer's fragility banner
...
date, with their own `k̂` per band. The dashboard's filter bar shows them
through its Tier floors select: set to off, each band a tier binds at shows
its tier-off "all" run at the primary `k` — the trade sections, the header's
trade count and the `k̂` chart all switch to it, and the scenario explorer
switches to the tier-off cells at every `k` (its Same-title row is the same
either way) — while a band whose floor is
0.30 or more shows its one run under both settings. They have no flag of their own:
`--no-band-sweep` skips them with the grid. A tier-off result is the model's
own extrapolation below the thresholds live trading applies — nothing about
it reaches the live bot, which always applies both tiers.
...
the band's floor is layered through, and the keyword-only `tier_floors`
switch the tier-floors-off runs throw (only an explicit `False` drops the
tiers), but no live caller (`scanner.py`, `strategy.py`, `trader.py`,
`main.py`) ever passes either or reads a band constant — see `CLAUDE.md`'s
`test_ast_live_path_reads_no_band`.
