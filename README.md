...
python3 -m kalshi_betting.backtest --no-band-sweep           # skip the spread-band grid (and its tier-floors-off runs); the dashboard's scenario explorer has nothing to show
```
...
show. The explorer's fragility banner
...
date, with their own `k̂` per band. They have no flag of their own:
