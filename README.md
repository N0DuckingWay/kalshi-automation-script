...
backtest.py (CLI)
  ...
  └─ dashboard.generate_dashboard()          — write HTML report: a k section and two k̂ cards
                                                ...
                                                filter bar whose every band x k x size cap x Kalshi
                                                category x tag view is computed here: one walk over the
                                                scenario grid reads each CapSweep cell (simulating it),
                                                packs one chunk per distinct trade list and keeps the k
                                                section's rows and curves at every k and cap, streamed
                                                into the page beside a base block and inflated by a
                                                small script only when chosen
```
